...
$(document).ready(function(){    
	...
	$('#filter_golongan').change(function(){
...
function getFilter(){
	...
	if(gol=='nol'){
		//alert('Mohon Pilih Golongan Terlebih Dahulu');
		 $.Notify({
			caption: "Warning",
			content: "Mohon Pilih Golongan Terlebih Dahulu",
		...
		$.ajax({
			...
			data  : { gol:gol, bid:bid, kel:kel, sub:sub, brg:brg, reg:reg},
			cache : false,
			success: function(datas) {
...
function loadAsetMenu(){
	...
	$( "#grafik" ).animate({width: "0px"}, 200);	
}

function loadDashboard(){
	clearMarker();
	disableAddData();
	    $( "#tematik" ).animate({width: "0px"}, 200);
    $( "#pencarianUnit" ).animate({width: "0px"}, 200);
	$( "#pencarian" ).animate({width: "0px"}, 200);
	$( "#legends" ).animate({width: "0px"}, 200);
	$( "#grafik" ).css("margin", "0px");
	$( "#grafik" ).animate({width: "100%",margin: "0px"}, 200);
